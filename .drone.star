def main(ctx):
  ...
  cmd.append('git fetch gh-token && git checkout gh-token/main')
  for state in data:
    for project in data[state]:
      name=project.split('/')[1]
      cmd.append("wget -O projects/%s/%s.md -nv https://raw.githubusercontent.com/%s/master/README.md" % (state, name, project))
